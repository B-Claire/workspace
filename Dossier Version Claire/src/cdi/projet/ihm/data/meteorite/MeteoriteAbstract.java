...
public abstract class MeteoriteAbstract implements Meteorite {
	...
	public MeteoriteAbstract(JPanel p, Meteorite m, Avion pl) {//constructeur
		...
		this.precedent = m;
		this.plane = pl;
		this.type = toString();//pour type meteorite 		
	...
	public void dessiner (Graphics g, boolean pause) {
		...
		if (this.posX != Integer.MIN_VALUE) { // si meteorite deja crée
			...
			if (nbEmptyStep < WHITE_STEP) { // si emptystep =0 emplacement libre pour repo meteorite
				...
				if(this.detruit) {
					...
					if (i<this.pan.getWidth()/10 || (i>=(this.pan.getWidth()*2)/10 && i<(this.pan.getWidth()*3)/10) || i>=(this.pan.getWidth()*4)/10) {//zigzag
		...
		if (posY>=this.pan.getHeight() && !this.detruit) {//calcul score
		GetscoreValue();
	...
	public void verifierImpacte() {//mecanisme impact
